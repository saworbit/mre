...
if not exist "%ROOT%\\reaper_mre\\progs.dat" (
  echo ERROR: reaper_mre\\progs.dat not found in %ROOT%reaper_mre
  echo Build or copy the MRE progs.dat and try again.
  exit /b 1
)

echo Launching Reaper MRE...
echo Maxplayers: %MAXPLAYERS%  Map: %MAPNAME%
echo.

"%ROOT%\\quakespasm.exe" -basedir "%ROOT%" -game reaper_mre -listen %MAXPLAYERS% +maxplayers %MAXPLAYERS% +deathmatch 1 +map %MAPNAME%

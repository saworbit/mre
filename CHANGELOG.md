...
- **Flow Governor (Priority-Based Arbitration)** to prevent competing control loops from fighting each other:
  ...
  - **Entity fields** in `reaper_mre/botit_th.qc`: Added 4 Flow Governor fields (lines 263-270): `.fg_pri` (current driver priority 0-100, higher wins), `.fg_driver` (current driver ID: FG_DRV_* constants), `.fg_until` (commit time when driver expires), `.fg_goalDir` (goal direction chosen by driver).
  - **Driver constants** in `reaper_mre/botmove.qc`: Defined 7 driver IDs (lines 33-40): `FG_DRV_NONE` (no active driver), `FG_DRV_TERRAIN` (terrain trap resolver, pri 100), `FG_DRV_LOOP` (loop breaker, pri 90), `FG_DRV_VERTICAL` (vertical reposition, pri 80), `FG_DRV_INTENT` (intent director, pri 60), `FG_DRV_ROUTE` (route memory, pri 40), `FG_DRV_ROAM` (default roam, pri 20).
  - **Core governor functions** in `reaper_mre/botmove.qc`: `FG_Tick()` (lines 2724-2732) releases expired control and resets governor state. `FG_Request()` (lines 2736-2785) handles control requests with priority arbitration—higher priority overrides lower, equal priority doesn't churn while committed. Breaks steering commit when new driver takes over. Includes debug logging at LOG_VERBOSE level showing driver takeovers.
  - **Terrain Trap Driver refactor** in `reaper_mre/botmove.qc`: Modified `Bot_HandleTerrainTrap()` (lines 2947-2959) to use `FG_Request(bot, 100.0, FG_DRV_TERRAIN, escape, 1.2)` instead of directly setting `feeler_commit_dir` and `feeler_commit_until`. Governor now handles commit window management.
  - **Loop Breaker Driver refactor** in `reaper_mre/botmove.qc`: Modified `Loop_BreakLoop()` (lines 3424-3430) to use `FG_Request(bot, 90.0, FG_DRV_LOOP, fwd_dir, 1.2)` instead of directly clearing `feeler_commit_until`. Removed direct commit manipulation.
  - **Vertical Reposition Driver** (NEW FEATURE) in `reaper_mre/botmove.qc`: Added `VT_PickRepositionDir()` (lines 3318-3410) to compute tangential strafe direction for finding angle on vertical stalemate. If too close to enemy (<256u), blends outward + tangent and picks clearer direction. If at good distance, strafes around enemy (tangL or tangR) to find LOS. Added `VT_Driver()` (lines 3413-3437) to request control when `VT_VerticalMode()` detects vertical stalemate (no LOS, big dz, close XY or no progress). Uses `FG_Request(bot, 80.0, FG_DRV_VERTICAL, reposition_dir, 1.0)` for priority 80 control.
  - **Intent Director Driver** (NEW FEATURE) in `reaper_mre/botmove.qc`: Added `Intent_Driver()` (lines 3216-3294) to translate high-level intent into movement goals. FIGHT intent → move toward enemy, CHASE intent → move toward last known enemy position, UNSTICK intent → use feeler best direction, EXPLORE/RESUPPLY intent → use feeler best direction. Uses `FG_Request(bot, 60.0, FG_DRV_INTENT, goal, 0.5)` for priority 60 control.
  - **Escalation refactor** in `reaper_mre/botmove.qc`: Modified `Bot_EscalateIntent()` (lines 3117-3189) to ONLY change intent, not movement state. Removed direct `stuck_mode` and `escape_until_time` setting from FIGHT→UNSTICK (lines 3121-3122 deleted) and EXPLORE→UNSTICK (lines 3168-3169 deleted). Removed `feeler_commit_until` clearing from UNSTICK→EXPLORE (line 3186 deleted). Intent changes drive movement via Intent_Driver, not direct overrides.
  - **Think loop integration** in `reaper_mre/botmove.qc`: Added `FG_Tick()` call after `Bot_ProgressTick()` to release expired control (lines 4985-4988). Added driver request calls after intent escalation (lines 5064-5078): `VT_Driver(self)` for vertical reposition (pri 80), `Intent_Driver(self)` for intent-based goals (pri 60). Added Flow Governor goal override (lines 5097-5134): if `fg_driver != FG_DRV_NONE`, use `fg_goalDir` for `ideal_yaw`, else use normal waypoint navigation with racing line smoothing.
  - **Forward declarations** in `reaper_mre/bot_defs.qc`: Added 7 function prototypes (lines 49-59): `Intent_Driver()`, `FG_Tick()`, `FG_Request()`, `VT_HasLOS()`, `VT_VerticalMode()`, `VT_PickRepositionDir()`, `VT_Driver()`.
  - **Result:** Competing control loops now arbitrated cleanly. Highest-priority driver wins, no oscillation. Terrain trap (pri 100) > Loop breaker (pri 90) > Vertical reposition (pri 80) > Intent director (pri 60) > Waypoint navigation (baseline). Short commit windows (0.5-1.2s) prevent lock-in while avoiding churn. New vertical reposition driver breaks "under platform" stalemates by tangentially strafing to find LOS. Intent escalation now tactical-only (changes intent), movement handled by Intent_Driver via Governor. Clean separation of concerns: tactical reasoning (intent) vs movement execution (Governor + TraceFlow).

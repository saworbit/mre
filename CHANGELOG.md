...
- **Intent Escalation System** to prevent stuck-in-valid-behavior loops (endless chasing, resupply loops, vertical stalemates):
  - **Entity fields** in `reaper_mre/botit_th.qc`: Added 7 intent tracking fields (lines 252-262): `.bot_intent` (current high-level intent: Fight/Chase/Resupply/Explore/Unstick), `.bot_intent_start` (when intent began for timeout checks), `.bot_intent_fail` (accumulates when intent not succeeding), `.bot_last_pickup` (last item pickup time), `.bot_last_enemy_seen` (last LOS to enemy), `.bot_last_damage_dealt` (optional damage signal), `.bot_intent_next_update` (4Hz rate limiter).
  - **Intent constants** in `reaper_mre/botmove.qc`: Defined 5 intent types and update frequency (lines 24-29): `BOT_INTENT_FIGHT` (engaging visible enemy), `BOT_INTENT_CHASE` (pursuing without LOS), `BOT_INTENT_RESUPPLY` (seeking health/armor/ammo), `BOT_INTENT_EXPLORE` (roaming for pickups/contact), `BOT_INTENT_UNSTICK` (escape stuck/reposition), `BOT_INTENT_UPDATE_HZ = 4.0` (evaluation frequency).
  - **Success signal functions** in `reaper_mre/botmove.qc`: `Bot_IntentSucceeded()` (lines 2897-2955) checks success per intent: RESUPPLY succeeds if pickup in last 2s, EXPLORE succeeds if pickup in 3s OR progress in 1s, CHASE succeeds if enemy seen in 1.5s, FIGHT succeeds if enemy visible in 1s, UNSTICK succeeds if progress in 0.8s. `Bot_IntentTimeout()` (lines 2958-2966) returns timeout thresholds: FIGHT 2.5s, CHASE 4s, RESUPPLY 5s, EXPLORE 6s, UNSTICK 2s.
  - **Fail accumulation** in `reaper_mre/botmove.qc`: `Bot_UpdateIntentFail()` (lines 2969-2996) accumulates fail score by 0.25/update when failing, decays by 0.25 when succeeding, spikes by 0.75 if loop detected (behavioral pathology) or hard stuck (movement pathology). Fail score gates escalation.
  - **Escalation graph** in `reaper_mre/botmove.qc`: `Bot_EscalateIntent()` (lines 2999-3107) switches intents when fail score ≥2.0 OR timeout exceeded. Escalation paths: FIGHT → UNSTICK (reposition for vertical stalemate), CHASE → RESUPPLY/EXPLORE (lost enemy, give up pursuit), RESUPPLY → EXPLORE (can't reach items), EXPLORE → UNSTICK (no pickups, force exit), UNSTICK → EXPLORE (still stuck, try different escape). Resets fail score and start time on escalation.
  - **Pickup hook** in `reaper_mre/botsignl.qc`: `Bot_OnPickup()` call added to `bot_toucheditem()` (line 120). Reduces fail score by 1.0 on pickup (progress signal for RESUPPLY/EXPLORE intents).
  - **Enemy sighting hook** in `reaper_mre/bot_ai.qc`: `Bot_OnEnemySeen()` call added when `enemyvisible()` returns TRUE (line 1309). Reduces fail score by 0.5 when enemy seen (progress signal for FIGHT/CHASE intents).
  - **Think loop integration** in `reaper_mre/botmove.qc`: Intent classification and escalation runs at 4Hz (lines 4715-4772), AFTER terrain trap resolver and loop breaker, BEFORE goal selection. Order: Progress tick → Loop sig push → Terrain trap → Loop breaker → **Intent escalation** → Goal/waypoint code. Classifies intent based on enemy/resources, updates fail metrics, escalates if needed.
  - **Forward declarations** in `reaper_mre/bot_defs.qc` and `reaper_mre/botmove.qc`: Added 6 function prototypes in bot_defs.qc (lines 42-48), added Loop_InLoop forward declaration in botmove.qc (line 13).
  - **Result:** Bots now automatically "switch gears" when stuck in valid behaviors: vertical stalemate (FIGHT → UNSTICK reposition), lost enemy (CHASE → EXPLORE), unreachable items (RESUPPLY → EXPLORE), no pickups (EXPLORE → UNSTICK), persistent stuck (UNSTICK → EXPLORE with fail memory). Prevents infinite loops in correct tactical behaviors that don't produce results. Runs at 4Hz to avoid intent churn. Success signals (pickup, enemy seen, progress) drive fail score accumulation/decay.
- **General Terrain Trap Resolver** for escaping movement pathologies (shallow water, lips, greebles, micro-basins):
  - **Detection logic** in `reaper_mre/botmove.qc`: `Bot_IsTerrainTrap()` function (lines 2700-2722) implements multi-condition detection. Checks: (1) No progress for 1.5s (`time - last_progress_time`), AND (2a) Shallow water (`waterlevel == 1`) for DM2 ankle water traps, OR (2b) General micro-trap heuristic (`velocity > 40 && stuck_score > 1`) for lips/greebles/basins. This failsafe layer detects when waypoints/intent/steering are all correct but terrain geometry blocks progress.
  - **Escape gradient selection** in `reaper_mre/botmove.qc`: `Bot_PickTerrainEscape()` function (lines 2726-2819) samples 7 radial directions (0°, ±30°, ±60°, ±90°) with exit gradient scoring. Formula: `score = clearance + (height × 0.01) - (fail_memory × 1.2)`. Prefers higher ground (climbing out of basin), open space (clearance ≥60%), and avoids recently failed directions via `GetDirectionalPenalty()`. Reuses existing `SampleGroundHeight()` helper for ground height sampling.
  - **Hard override handler** in `reaper_mre/botmove.qc`: `Bot_HandleTerrainTrap()` function (lines 2822-2875) implements commit-and-escape behavior. Breaks existing `feeler_commit_until` (allows new escape direction), picks escape via `Bot_PickTerrainEscape()`, records attempt in directional fail memory via `RecordDirectionalFail()`, commits hard for 1.2s to avoid trap re-entry, forces `BOT_MODE_UNSTICK` for 1.2s to prevent pathfinding interference. Includes debug logging (LOG_CRITICAL level) showing waterlevel, velocity, stuck_score, and no-progress duration.
  - **Integration ordering** in `reaper_mre/botmove.qc`: Added `Bot_HandleTerrainTrap()` call at line 4452, BEFORE `Loop_BreakLoop()` (line 4456). Execution order: (1) Progress tick, (2) Loop signature push, (3) **Terrain trap resolver** (NEW), (4) Loop breaker, (5) Unstuck mode check, (6) Water trap (legacy). Runs before loop breaker because terrain traps can cause loops, but not all loops are terrain traps—terrain traps are movement pathology, loops are behavioral pathology.
  - **Forward declarations** in `reaper_mre/bot_defs.qc`: Added 3 function prototypes (lines 37-40): `Bot_IsTerrainTrap()`, `Bot_PickTerrainEscape()`, `Bot_HandleTerrainTrap()`.
  - **Result:** Bots now escape DM2 ankle water, recessed lips, decorative greebles, and micro-basins using exit gradient heuristic (pick direction and commit, like humans). General micro-trap detection (`velocity > 40 && stuck_score > 1`) catches any "moving but not progressing" pathology beyond just water. Runs as first failsafe layer before loop detection. Works alongside existing shallow water handler (`HandleWaterTrap` at line 4462) for backwards compatibility, but terrain trap resolver is more general and handles all terrain pathologies.

## 2026-01-10

- **PID Aiming Controller** for smooth, human-like aim instead of robotic snap-to-target:
  - **Entity fields** in `reaper_mre/botit_th.qc`: Added PID state tracking fields (lines 242-250): `.pid_error_sum_x/y` (integral accumulators for I term), `.pid_last_error_x/y` (derivative memory for D term), `.pid_last_update` (timestamp for delta time calculation).
  - **Angle wrapping** in `reaper_mre/botmath.qc`: `Math_AngleDiff()` function (lines 500-512) normalizes angle differences to [-180, 180] range. Handles 359° → 1° = +2° correctly (not -358°).
  - **PID controller core** in `reaper_mre/botmath.qc`: `Math_PID()` function (lines 523-615) implements physics-based control loop. Formula: Output = (Kp × Error) + (Ki × ∫Error dt) + (Kd × dError/dt). Default tuning: Kp=10.0 (proportional response), Ki=0.5 (integral for micro-corrections), Kd=0.8 (derivative for damping). Anti-windup protection resets I term if error >45°. Integral clamping (±10 limit) prevents spin-up. Separate axis processing for pitch (X) and yaw (Y).
  - **Aim update integration** in `reaper_mre/botmath.qc`: `Bot_UpdateAimPID()` function (lines 621-667) calculates delta time, applies PID smoothing to both yaw and pitch, clamps pitch to [-45, 45] degrees, keeps head level (no roll).
  - **BotPostThink integration** in `reaper_mre/botthink.qc`: Added `Bot_UpdateAimPID()` call (line 746) at end of BotPostThink(). Runs once per frame per bot, reads ideal_yaw/ideal_pitch set by bot AI, smoothly transitions to targets with realistic acceleration/deceleration.
  - **Conflict prevention** in `reaper_mre/botfight.qc`: Modified `ChangePitch()` (lines 1029-1036) to skip processing for bots (checks `.isbot` flag), avoiding double-application of aim updates.
  - **Forward declarations** in `reaper_mre/bot_defs.qc`: Added `Math_AngleDiff()`, `Math_PID()`, `Bot_UpdateAimPID()` prototypes (lines 20-23).
  - **Result:** Bots now aim with smooth physics-based transitions instead of instant snapping. Micro-corrections from I term improve long-range accuracy. D term prevents overshooting for natural deceleration. Tunable personality: high Kp/low Kd = snappy aim with slight overshoot (realistic flick shots), low Kp/high Kd = smooth cinematic aim. All existing ChangeYaw() calls continue to set ideal_yaw; PID controller smoothly interpolates to target.
